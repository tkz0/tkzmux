// Guards the seam TKZ-86 depends on: nothing under `Sources/ClaudeBridge/Codex/` may name the
...
// to prevent (see `ClaudeBridge.swift`'s header). Mirrors `HookHygieneTests`, which does the same
...
@Suite struct CodexHygieneTests {
    static var moduleDirectory: URL {
        URL(fileURLWithPath: #filePath)          // Tests/ClaudeBridgeTests/CodexHygieneTests.swift
            .deletingLastPathComponent()          // Tests/ClaudeBridgeTests
            .deletingLastPathComponent()          // Tests
            .deletingLastPathComponent()          // <repo>
            .appendingPathComponent("Sources/ClaudeBridge/Codex")
    }
    ...
    /// The module's own name, which is unavoidable in a path and says nothing about an agent.
    ///
    /// This directory lives *inside* a module still called `ClaudeBridge`, so any file that cites a
    /// path — a fixture, a sibling source file — necessarily writes that word. Failing on it would
    /// force comments to describe paths instead of naming them, which is worse than the thing the
    /// rule is protecting against. The exception disappears when TKZ-88 renames the module to
    /// `AgentBridge`, which is one more small argument for doing that rename.
    private static let moduleNameExceptions = ["claudebridge"]
    ...
    @Test func namesNothingFromTheOtherAgent() throws {
        ...
        #expect(offences.isEmpty, "Sources/ClaudeBridge/Codex must not name the other agent: \(offences)")
    }
    ...
    @Test func theScanWouldCatchAnOffender() {
        ...
        #expect(!offends("// see Tests/ClaudeBridgeTests/Fixtures/codex/hooks-json-verified.json"))
        #expect(offends("// ClaudeBridge's ClaudeAdapter does this differently"))
    }
}
