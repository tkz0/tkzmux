// TkzCore — theme tokens. See docs/design.md → App architecture → Theme.
//
// Source of truth: the Claude Design file `Terminal Main Window.dc.html` (project 452d6955-…), five artboards:
// 2c Midnight indigo (default), 2a Graphite, 2b Warm charcoal, 1a Dark, 1b Light. Every preset is the same
// token set; nothing in the app may branch on the preset name. Extraction rule per token is listed next to
// the token, the resulting values are tabulated in design.md (generated by ThemeTests.printsDesignTable).
...
public struct Theme: Hashable, Sendable {
    ...
    public let foregroundDim: RGB         // ⌘K hint, chevrons, ＋ (identical to muted in 1a: that is what the artboard does)
...
extension Theme {
    /// 2c · Midnight indigo — the default.
    public static let midnightIndigo: Theme = {
        let fg = RGB(hex: 0xedf0fd), termFg = RGB(hex: 0xe2e6f8), dim = RGB(hex: 0x8890b4)
        let statusBar = RGB(hex: 0x1e2136)
        let working = RGB(hex: 0x4ade80), waiting = RGB(hex: 0xfbbf54), diffRemove = RGB(hex: 0xf28b8b)
        return Theme(
            preset: .midnightIndigo, isDark: true,
            windowBackground: RGB(hex: 0x141624),
            titlebar: RGB(rgb: 31, 34, 54, alpha: 0.95),
            sidebarBackground: RGB(hex: 0x1a1d31),
            terminalBackground: RGB(hex: 0x0e101c),
