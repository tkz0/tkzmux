...
| `showBoard` | ⇧⌘K | The Kanban board over the terminal; also the toolbar's calendar button, right after `>_`. Four fixed columns (To Do, In Progress, In Review, Done). A card is a prompt: its title and notes are what an agent is given. Give it a **group** and any Claude session in that group takes it when free; give it an **agent** (a session row) and that one does. An agent can hold any number of cards and works them one at a time in To Do order: when its turn ends (`Stop`) the card moves to In Review and the next is pasted in. Every session with a Claude running in it is an agent for its group's cards, with nothing to set up; the one exception is the session you are *in* (selected, board closed), which takes cards assigned to it but no group card, because a paste there would land in whatever you are typing. Nothing is sent while a permission prompt or question is up. *Agent ▸ Start New Agent* on a card runs `claude -w` in the card's group and assigns it. Drag cards between and within columns; double-click edits; `⋮` or right-click for agent, group, move, delete. Esc, ✕, the chord again, or picking a row in the sidebar returns to the terminal. After a relaunch a card that was In Progress is In Review, never re-sent |
