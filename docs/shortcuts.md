...
| `showFirstPrompt` | ⌥⌘P | Glass card over the terminal with the selected session's first prompt and Claude's recap, centred on the screen (design 2c.5); again or Esc closes. Scrolling the terminal never shows it |
...
| `runDevServer` | ⌃⌘R | The toolbar's `▶ pnpm dev` button as a chord: runs the selected row's dev server in a stacked split under the focused pane, or — while it runs, when the button reads `■` — sends it Ctrl-C. The command comes from the row's own checkout (a worktree row detects and runs in its worktree, never the main checkout): the group's remembered command, else the best guess from the root's `package.json` scripts (runner from `packageManager` or the lockfile), `deno.json`, `justfile`, `Makefile`, `Taskfile.yml`, or `cargo`/`go`/`swift`/`dotnet`/Django/`bin/dev`/`docker compose` — `dev`, then `start`, `serve`, `up`, `run`. The `▾` half lists every detected task; **picking one runs it and remembers it for the group**, so every worktree row of that repo uses it from then on. *Custom Command…* remembers anything detection cannot see (a `.sln`'s `dotnet watch run --project …`); *Reset to Detected* forgets it. Running again reuses the same pane; the listening port shows up in the status bar as usual |
