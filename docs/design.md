...
- **Toolchain**: `brew` zig stable **0.16.0** = Ghostty main minimum; Xcode 26.1 + `xcrun metal` present; `xterm-ghostty` terminfo already system-wide (cmux ships `Resources/terminfo/78/xterm-ghostty`); **JetBrains Mono not installed** (Menlo fallback exercised from day one); gh logged in as tkz0.
...
- **Theme** tokens (midnight indigo): window `#141624`, titlebar `rgba(31,34,54,.95)`, sidebar `#1a1d31`, terminal bg derived from 2c main, status bar from 2c footer, fg `#edf0fd`, muted `#98a0c2`/`#8890b4`, accent `#8b93f8`, working `#4ade80`, waiting `#fbbf54`, idle `rgba(255,255,255,.30)`, selection `rgba(139,147,248,.22)`, WT `#c3c8fd` on `rgba(139,147,248,.20)`, group edge example `#41c6a8`, diff `+#4ade80 −#f07a7a`. Other four presets extracted from the same HTML. Terminal palette derives from the theme.
